...
public class VideoController {
	...
	/**
	 * Elimina un vídeo determinadadoRecupera un determinado usuario de la BBDD
	 * @param id Id del usuario
	 * @return UserDTO
	 */
	@DeleteMapping(value="/private/video/{id}/{idUsuario}")
	@ApiOperation(value="Recupera las videotecas públicas",notes="Provee un mecanismo para recuperar las videotecas públicas")
	@ApiResponses(value={
		@ApiResponse(code=200,message="OK",response=Boolean.class),
		@ApiResponse(code=404,message="Not Found",response=ResponseError.class),
	...
	public ResponseEntity<?> deleteVideo(@PathVariable Long id,@PathVariable Long idUsuario) {

		Video video = this.manager.getVideo(id, idUsuario);
		
		if(video!=null && Boolean.TRUE.equals(this.manager.deleteVideo(id, idUsuario))) {
			// Se devuelve un HTTP 200	
			ResponseOperation<Object> response = new ResponseOperation<Object>();
			response.setStatus(HttpStatus.OK);
			response.setDescStatus("OK");
